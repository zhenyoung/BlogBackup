...
之前一直想了解一下HTML的一些常见的标签。最近有一点时间，开始学习一些常用的HTML标签以及CSS样式。由于我自己并不是一个专业的前端工程师，学习这些知识也仅仅是为了写博客时的一些格式。所以并不会涵盖所有的内容，知识粗略地学习一下
...
结构：HTML用于描述页面的结构，类似人的骨架，是看不到的；

表现：CSS用于控制页面中元素的样式，类似人的皮肤、衣服等，是可以被人看到的；

行为：JavaScript用于响应用户操作，类似人的行为动作等。
...
`<!DOCTYPE html>`告诉浏览器：网页是基于HTML5版本的。`<html></html>`是根标签，内部有head和body两个子标签。head标签内部有一个title子标签，写网页的标题。body标签内部即为正文内容。`<h1></h1>`内部写的标题是正文的一个一级标题

标签之中可以设置属性（字体的大小、颜色等），有些属性有属性值，有些没有。如果有属性值，则应该用引号引起来
...
当编码和解码所采用的字符集不同，就会出现乱码问题

常见字符集：

- ASCII：美国标准信息交换码,7位二进制位表示字符，共表示128个字符

- ISO88591：欧洲字符编码（对ASCII码的一种扩充），8位二进制位表示字符，共表示256个字符

- GB2312：中国字符编码（对ASCII码的一种扩充）
...
- em标签：`<em>emphasize</em>`表示”语音语调“的一个加重。在页面中不会独占一行的元素称为行内元素（或称内联元素，inline element）
- 强调标签：`<strong>strong</strong>`表示强调重要内容
...
无序列表使用`<ul></ul>`，有序列表使用`<ol></ol>`，使用`<li>list item</li>`表示列表项。

定义列表使用`<dl></dl>`，使用`<dt></dt>`表示定义的内容，使用`<dd></dd>`来对内容进行解释说明。
...
跳转到其他页面或者当前页面的其他位置，超链接也是一个行内元素。通过a标签的href属性可以跳转到目标路径。
...
| ------ | :----------------------------------------------------------: |
| src    |                          图片的地址                          |
| alt    | 图片的描述，默认不会显示，搜索引擎会根据alt中的内容来识别图片 |
| width  |                   图片的宽度（单位为像素）                   |
| height |                   图片的高度（单位为像素）                   |
...
# 6.CSS（Cascading Style Sheet，层叠样式表）介绍
